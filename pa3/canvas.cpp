...
{
    PatchNode* curr = canvas->top;
    bool select = false;
    while(curr != nullptr) {
        Line* l = curr->patch->head;
        if(curr->patch->x <= x ) {
            for(int i = curr->patch->x; i < x; i++) {
                if(l->next != nullptr){
                    l = l->next;
                } else {
                    l = nullptr;
        ...
        if(curr->patch->y <= y && l != nullptr) {
            if(curr->patch->y + l->data.length() > y){
                if(l->data.at(y - curr->patch->y) != ' '){
                    canvas->selected = curr;
                    select = true;
                    break;
                }
            }
...
void erase_pixel_at(Canvas *canvas, int x, int y)
{
    ...
    while(curr != nullptr) {
        ...
        if(curr->patch->y <= y && l != nullptr) {
            if(curr->patch->y + l->data.length() > y){
                if(l->data.at(y - curr->patch->y) != ' '){
                    l->data.at(y-curr->patch->y) = ' ';
                }
            }
        }
		for (const Line* li = curr->patch->head; li; li = li->next) {
			for (const char c : li->data) {
				if (c != ' ') {
					erasable = false;
				}
			}
		...
		if (erasable) {
			PatchNode* temp = curr;
			PatchNode* prev = temp->above;
			PatchNode* aft = temp->below;
			if(canvas->top == temp) {
			    canvas->top = aft;
                canvas->top = temp->below;
                if(aft!= nullptr) {
                    aft->above = prev;
                }
			    clear(temp->patch);
