...
bool getMarried(const char male[MAX_NAME_LENGTH], const char female[MAX_NAME_LENGTH]) {
    if(searchPerson(male) && searchPerson(female)) { // evaluate if both person exist
        Person* groom = searchPerson(male); // assign male as groom
        Person* bride = searchPerson(female); // assign female as bride
        if(!groom->spouse && !bride->spouse) { // assign new spouses
            groom-> spouse = bride;
            bride -> spouse = groom;
            cout << "Congratulations, " << groom->name << " and " << bride->name << " are married." << endl;
            return true;
        } else if(groom->spouse) { //bigamy error
            cout << "Error: groom already has a spouse!" << endl;
        } else if(bride-> spouse) {
            cout << "Error: bride already has a spouse!" << endl;
        }
    } else if(!searchPerson(male)) { // Error messages
        cout << "Error: groom does not exist!" << endl;
    } else if (!searchPerson(female)) {
        cout << "Error: bride does not exist!" << endl;
    } else {
...
bool addKid(const char father_name[MAX_NAME_LENGTH], const char mother_name[MAX_NAME_LENGTH], const char kid_name[MAX_NAME_LENGTH]) {
    ...
    if((sizeof(father->kid) / sizeof(kid)) <= MAX_KID_NUM && (sizeof(mother->kid) / sizeof(kid)) <= MAX_KID_NUM) {
